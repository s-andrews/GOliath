...
# usage perl process_geneontology_org_file --obo go-basic.obo --gene_name 1 --go_id 3 --gene_ontology_file gene_association.mgi 
...
while (my $line = <IN>){
	...
	my $gene_name = $fields[$gene_name_index];
	my $go_id = $fields[$go_id_index];
	
	if($count <= 5){
		print "gene_name is $gene_name\n";
		print "go_id is $go_id\n";
	}	
	...
	# If the GO category doesn't exist in the hash, create it. We need to look up the info from the obo file.
		...
		#if($count == 31){
		#	print "go_category_info 31 = @go_category_info \n";
		#}
	
		#print "go_category_info[0] = @go_category_info[0] \n";
		
		#if(@go_category_info[0] == 0){
		#	print "it's 0 = @go_category_info \n";
		#}
		
		next if(@go_category_info[0] eq 0);	
