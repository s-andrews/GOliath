...
# This is including the other relationships too, so the regulates, part of, negatively regulates etc.
...
OBO_LINE: while (my $line = <IN>){
	...
	elsif($line  =~ /^relationship:/){
		
		$line =~ s/^relationship: //;
		my @parent_info = split(/ ! /, $line);
		my $parent_id = $parent_info[0];
		$parent_id =~ s/part_of //;
		$parent_id =~ s/negatively_regulates //;
		$parent_id =~ s/positively_regulates //;
		$parent_id =~ s/regulates //;
...
$outfile = "$outfile.including_relatives.gmt";
	...
	if($go_id ne $info[2]){
		...
		while(@parents){
			...
			foreach my $parent(@parents){
				...
				# check that it does now exist and add the genes - we don't want the genes to be duplicated hence the check for whether they already exist.
